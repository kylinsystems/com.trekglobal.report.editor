...
import com.trekglobal.report.editor.FieldPosition;
import com.trekglobal.report.editor.PrintAreaBean;
 ...
 *
 */
public class WPrintFormatEditor implements EventListener<Event>, ValueChangeListener {

	private static final String PRINTFORMAT_ITEM_ATTR = "printFormatItem";
	private static final String PT = "pt";
	private static final String selectedStyle = "curser:pointer; border:dashed 1px blue;";
	private static final String normalStyle = "curser:pointer; border:dashed 1px black;";
	...
	private MPrintFormat m_printFormat = null;
	...
	/** Logger */
	public static CLogger log = CLogger.getCLogger(WPrintFormatEditor.class);
	...
	public void showEditorWindow() {
		try {
			...
			String title = Msg.getMsg(Env.getCtx(), "com.trekglobal.report.editor.action.EditorAction.label");
			...
			updatePreview();
			LayoutUtils.sendDeferLayoutEvent(printFormatForm.mainLayout, 100);
		} catch (Exception ex) {
	...
	private void updatePreview() {
		...
		printableArea.setWidth(getActualPaperWidth() + PT);
		printableArea.setHeight(getActualPaperHeight() + PT);
		...
		for (MPrintFormatItem pfItem : formatItems) {
			...
			child.addEventListener(Events.ON_DROP, this);
			...
			fieldDiv.addEventListener(Events.ON_CLICK, this);
		...
		if (m_printFormat.isStandardHeaderFooter()) {
			...
			line.setWidth(getActualPaperWidth()+4+PT);
	...
	public void onEvent(Event event) throws Exception {
		...
		if (event.getName().equals(Events.ON_SELECT)
				&& event.getTarget() instanceof Listbox) {
	...
	private void onDrop(DropEvent de)
	{
		...
		if (startItem != null && endItem != null) {
			...
			if ("headerMargin".equals(child.getId())) {
				...
				m_printFormat.setHeaderMargin(new Float(ptY - topRef).intValue());
				updatePreview();
				return;
			} else if ("footerMargin".equals(child.getId())) {
				float ptY = PrintFormatEditorUtil.pixelToPointValue(de.getY());
				m_printFormat.setFooterMargin(new Float(paperHeight - bottomRef - ptY).intValue());
				updatePreview();
				return;
			}
	...
	private void updateLists(MPrintFormatItem selectedItem) {
		...
		printFormatForm.printedItemListbox.removeEventListener(Events.ON_SELECT, this);
		printFormatForm.printedItemListbox.addEventListener(Events.ON_SELECT, this);
